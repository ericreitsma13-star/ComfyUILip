#!/usr/bin/env python3
"""
LTX 2.3 FP8 Checkpoint Lip-Sync Test
Uses transformer-only FP8 checkpoint (24GB) with separate text encoder + VAEs
"""
import json, uuid, time, urllib.request, urllib.parse, random, sys, os
from pathlib import Path

COMFYUI_URL = "http://127.0.0.1:8188"

def build_workflow(image, audio, prompt, w=704, h=1280, fps=24, dur=8, seed=42, i2v=0.7, lora=0.3):
    fc = int(((dur * fps - 1) / 8) * 8 + 1)
    return {
        "1": {"class_type": "LowVRAMCheckpointLoader", "inputs": {"ckpt_name": "ltx-2.3-22b-distilled-1.1_transformer_only_fp8_scaled.safetensors"}, "_meta": {"title": "Load FP8 Transformer"}},
        "2": {"class_type": "LoraLoaderModelOnly", "inputs": {"model": ["1", 0], "lora_name": "ltx-2.3-22b-distilled-lora-384-1.1.safetensors", "strength_model": lora}, "_meta": {"title": "LoRA"}},
        ...
        "35": {"class_type": "LTXVImgToVideoInplace", "inputs": {"vae": ["4", 0], "image": ["25", 0], "latent": ["30", 0], "strength": i2v, "bypass": False}, "_meta": {"title": "I2V"}},
        ...
        "51": {"class_type": "CLIPTextEncode", "inputs": {"clip": ["5", 0], "text": "ugly, distorted face, blurry, cartoon, bad anatomy, crooked head, low quality"}, "_meta": {"title": "Negative"}},
        ...
        "81": {"class_type": "LTXMotionSaveVideo", "inputs": {"video": ["80", 0], "filename_prefix": "ltx_fp8"}, "_meta": {"title": "Save"}},
    }
...
def poll_complete(pid, timeout=900):
    start = time.time()
...
def main():
    ...
    p.add_argument("--lora", type=float, default=0.3)
    args = p.parse_args()
    ...
    print(f"FP8 Checkpoint Test")
    print(f"  Image: {args.image}")
    print(f"  Audio: {args.audio}")
    print(f"  {args.width}x{args.height} @ {args.fps}fps, {args.duration}s")
    print(f"  LoRA: {args.lora}, I2V: {args.i2v}, Seed: {args.seed}")

    wf = build_workflow(args.image, args.audio, args.prompt, args.width, args.height,
                        args.fps, args.duration, args.seed, args.i2v, args.lora)
    pid = queue_prompt(wf)
    print(f"  Queued: {pid}")

    result = poll_complete(pid)
    if result:
        print(f"  Complete!")
    else:
        print("  TIMEOUT")
